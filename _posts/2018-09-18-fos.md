...
The function `mapKeys` has a *dependent function type*. This is an interesting type, because it has an internal dependency: `(k: KeyGen, ss: List[String]) -> List[k.Key]`. In Scala 2, can't express this directly (we'd have to go through a trait with an apply method). Scala 3 (dotty) [introduces these dependent function types](http://dotty.epfl.ch/docs/reference/new-types/dependent-function-types.html) at the language level; it's done with a similar trick to what we just saw. In dotty, the intention was to have everything map to a simple object type; this has been formalized in a calculus called DOT, (path-)Dependent Object Types. 
