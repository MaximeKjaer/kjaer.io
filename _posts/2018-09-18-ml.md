...
In this course, we'll always denote the dataset as a $N \times D$ matrix $\pmb{X}$, where $N$ is the data size and $D$ is the dimensionality, or the number of features. We'll always use subscript $n$ for data point, and $d$ for feature. The labels, if any, are denoted in a $\pmb{y}$ vector, and the weights are denoted by $\pmb{w}$:
...
$\pmb{w} = (w_0, w_1)$ are the parameters of the model.
...
A strictly convex function has a unique global minimum $\pmb{w}^*$. For convex functions, every local minimum is a global minimum. This makes it a desirable property for loss functions, since it means that cost function optimization is guaranteed to find the global minimum.
...
Given a cost function (or loss function) $\cost{\pmb{w}}$, we wish to find $\pmb{w}^*$ which minimizes the cost:

$$
\min_{\pmb{w}}{\cost{\pmb{w}}}, \quad\text{ subject to } \pmb{w} \in \mathbb{R}^D
$$
...
The global minimum $\pmb{w}^*$ is defined by getting rid of the radius $\epsilon$ and comparing to all other values:

$$
\cost{\pmb{w}^*} \le \cost{\pmb{w}}, \qquad \forall\pmb{w}\in\mathbb{R}^D
$$
...
$$
\cost{\pmb{w}^*} < \cost{\pmb{w}}, \qquad \forall\pmb{w}\in\mathbb{R}^D\setminus\set{\pmb{w^*}}
$$
...
Gradient descent is an iterative algorithm. We start from a candidate $\pmb{w}^{(t)}$, and iterate.De
...
As an example, we will take an analytical look at a gradient descent, in order to understand its behavior and components. We will do gradient descent on a 1-parameter model ($D=1$ and $\pmb{w} = [w_0]$), in which we minimize the MSE, which is defined as follows:
...
In this case, we've managed to find to this exact problem analytically from gradient descent. This sequence is guaranteed to converge to $\pmb{w}^* = \bar{y}$[^optimality-linear-mse]. This would set the cost function to 0, which is the minimum.
...
$$
f_{\pmb{w}}(\pmb{x}_n)=\pmb{x}_n^T \pmb{w}
$$
...
\frac{\partial}{\partial\pmb{w}_d} \cost{\pmb{w}}
	= -\frac{1}{2N} \sum_{n=1}^N {2(y_n - \pmb{x}_n^T \pmb{w}) x_{nd}}
...
$$
\mathcal{L}_n(\pmb{w}) = \frac{1}{2} \left(y_n -\pmb{x}_n^T \pmb{w}\right)^2 \\
\nabla\mathcal{L}_n(\pmb{w}) = (-\pmb{x}_n^T) (y_n-\pmb{x}_n^T \pmb{w})
$$

Note that $\pmb{x}_n^T \in\mathbb{R}^D$, and $e_n = (y_n-\pmb{x}_n^T \pmb{w})\in\mathbb{R}$. Computational complexity for this is $\mathcal{O}(D)$. 
...
$$
P_\mathcal{C}(\pmb{w}') := \argmin_{\pmb{v}\in\mathcal{C}}{\norm{\pmb{v-w'}}}
$$
...
$$
\text{optimum at }\pmb{w}^*, \quad \mathcal{L} \text{ convex} 
\implies 
\cost{\pmb{w}^*} = 0
$$
...
$$
\cost{\pmb{w}^*} = 0, \quad \mathcal{L} \text{ convex} 
\implies 
\text{optimum at }\pmb{w}^*
$$

This tells us when $\pmb{w}^*$ is an optimum, but says nothing about whether it's a minimum or a maximum. To know about that, we must look at the second derivative, or in the general case where $D > 1$, the Hessian. The Hessian is the matrix of second derivatives, defined as follows:

$$
\pmb{H}_{ij} = \frac{\partial^2\mathcal{L}}{\partial w_i \partial w_j}
$$
...
$$
\pmb{H}(\pmb{w}^*) := \frac{\partial^2 \cost{\pmb{w}^*}}{\partial\pmb{w}\partial\pmb{w}^T} \text{ positive semidefinite}
\implies
\pmb{w}^* \text{ is a minimum}
$$
...
$$
\pmb{H} \text{ positive semidefinite}
\iff
...
- If $D>N$, we always have $\text{rank}(\pmb{X}) < D$ (since column and row rank are the same, which implies that $\text{rank}(\pmb{X}) \le N < D$).
...
For a Gaussian random *vector*, we have $\pmb{y} \in \mathbb{R}^N$ (instead of a single random variable in $\mathbb{R}$). The vector has mean $\pmb{\mu}$ and covariance $\pmb{\Sigma}$ (which is positive semi-definite), and its distribution is given by:
...
\pmb{\mathcal{N}}(\pmb{y} \mid \pmb{\mu}, \pmb{\Sigma}) = 
    ...
    \exp{\left[ -\frac{1}{2} (\pmb{y} - \pmb{\mu})^T \pmb{\Sigma}^{-1} (\pmb{y} - \pmb{\mu}) \right]}
$$
...
The model $\pmb{w}$ is, as always, unknown. But we can try to do a thought experiment: if we did know the model $\pmb{w}$ the data $\pmb{X}$, in a system without the noise $\epsilon_n$, we would know the labels $\pmb{y}$ with 100% certainty. The only thing that prevents that is the noise $\epsilon_n$; therefore, given the model and data, the probability distribution of seeing a certain $\pmb{y}$ is only given by all the noise sources $\epsilon_n$. Since they are generated independently in each dimension, we can take the product of these noise sources. 

Therefore, given $N$ samples, the **likelihood** of the data vector $\pmb{y} = (y_1, \dots, y_n)$ given the model $\pmb{w}$ and the input $\pmb{X}$ is:

$$
p(\pmb{y} \mid \pmb{X}, \pmb{w}) 
    = \prod_{n=1}^N {p(y_n \mid \pmb{x}_n, \pmb{w})}
    = \prod_{n=1}^N {\normal{y_n \mid \pmb{x}_n^T\pmb{w}, \sigma^2}}
$$
...
$$
\mathcal{L}_{\text{MSE}}(\pmb{w}) := \frac{1}{N} \sum_{n=1}^N \left(y_n - \pmb{x}_n^T\pmb{w}\right)^2
$$
...
$$
\pmb{w}_\text{MLE} \overset{p}{\longrightarrow} \pmb{w}_\text{true}
$$
...
\frac{1}{\sqrt{N}} \normal{\pmb{w}_{\text{MLE}} \mid \pmb{0}, \pmb{F}^{-1}(\pmb{w}_{\text{true}})}
$$
...
$$
\norm{\pmb{w}}_1 := \sum_i{\abs{w_i}}
$$

If we draw out a constant value of the $L_1$ norm, we get a sort of "ball". Below, we've graphed $\left\\{ \pmb{w} : \norm{\pmb{w}}_1 \le 5 \right\\}$.
...
$$
S = \set{
    (\pmb{x}_n, y_n) \overset{\text{i.i.d}}{\sim} \mathcal{D}
}_{n=1}^N
$$
...
We assume that $\pmb{x}$ is generated according to some fixed but unknown distribution $\mathcal{D}_{\pmb{x}}$. We'll be working with square loss $\mathcal{l}(y, f(\pmb{x})) = \frac{1}{2}(y-f(\pmb{x}))^2$. We will denote the joint distribution on pairs $(\pmb{x}, y)$ as $\mathcal{D}$.

$$
\begin{align}
\epsilon     & \sim \mathcal{D}_\epsilon \\
\pmb{x}      & \sim \mathcal{D}_x \\
(\pmb{x}, y) & \sim \mathcal{D} \\
\end{align}
$$
...
$$
\mathcal{l}(y_0, f_{\Strain}(\pmb{x}_0))
=
...
$$
\newcommand{\otherconstantterm}{\expectsub{\Strain'\sim\mathcal{D}}{f_{\Strain'}(\pmb{x}_0)}}
...
\expectsub{\Strain \sim \mathcal{D}}{\left(
    \underbrace{f(\pmb{x}_0) - \otherconstantterm}_u 
+   \underbrace{\otherconstantterm - f_{\Strain}(\pmb{x}_0)}_v
\right)^2}
$$
...
& 2 \cdot \expectsub{\Strain\sim\mathcal{D}} {
    ...
    \right) \cdot \left(
        \otherconstantterm - f_{\Strain}(\pmb{x}_0)
    \right)
} \\
& = 2 \cdot \left(
    f(\pmb{x}_0) - \otherconstantterm 
...
& = 2 \cdot \left(
    f(\pmb{x}_0) - \otherconstantterm 
\right) \cdot \left(
    \otherconstantterm - \expectsub{\Strain\sim\mathcal{D}}{f_{\Strain}(\pmb{x}_0)}
\right) \\
& = 0 \\
\end{align} 
$$
...
- The **bias term** is a non-negative term that tells us how far we are from the true value, in expectation. It's the square loss between the true value $f(\pmb{x}_0)$ and the expected prediction $\otherconstantterm$, where the expectation is over the training sets. As [we discussed above](#bias-variance-decomposition), with a simple model we will not find a good fit on average, which means the bias will be large, which adds to the error we observe.
...
$$
\Strain = \set{(\pmb{x}_n, y_n)}_{n=1}^N,
\qquad \pmb{x}_n \in \mathbb{R}^d,\ y_n \in\mathbb{R}
$$
...
If we knew the underlying distribution $\mathcal{D}$, then it would be clear how we could measure the probability of error. We have a correct prediction when $y - f(\pmb{x}) = 0$, and an incorrect one otherwise, so:

$$
\expectsub{\mathcal{D}}{\mathbb{I}\set{y-f(\pmb{x}) \ne 0}} = \mathbb{P}(y-f(\pmb{x}) \ne 0)
$$
...
How do we use this? Let's consider binary classification, with labels 0 and 1. Given a training set, we learn a weight vector $\pmb{w}$. Given a new feature vector $\pmb{x}$, the *probability* of the class labels given $\pmb{x}$ are:
...
$$
\argmax_{\pmb{w}}{p(\pmb{y}, \pmb{X} \mid \pmb{w})}
$$
...
$$
p(\pmb{y}, \pmb{X} \mid \pmb{w}) = p(\pmb{X}\mid\pmb{w})p(\pmb{y} \mid \pmb{X}, \pmb{w}) = p(\pmb{X})p(\pmb{y} \mid \pmb{X}, \pmb{w})
$$

As we're trying to get the argmax over the weights, we can discard $p(\pmb{X})$ as it doesn't depend on $\pmb{w}$. Therefore:
...
p(\pmb{y} \mid \pmb{X}, \pmb{w})
    ...
    & = \prod_{n=1}^N{p(y_n \mid \pmb{x}_n, \pmb{w})} \\
    & = \prod_{n=1}^N{\sigma(\pmb{x}_n^T \pmb{w})^{y_n} (1-\sigma(\pmb{x}_n^T \pmb{w})^{1-y_n})} \\
\end{align}
$$
...
### Newton's method
Gradient descent is a *first-order* method, using only the first derivative of the cost function. We can get a more powerful optimization algorithm using the second derivative. This is based on the idea of Taylor expansions. The 2<sup>nd</sup> order Taylor expansion of the cost, around $\pmb{w}^*$, is:

$$
\cost{\pmb{w}} \approx \cost{\pmb{w}^*}^T(\pmb{w}-\pmb{w}^*) + \frac{1}{2}(\pmb{w}-\pmb{w}^*)^T \pmb{H}(\pmb{w}^*)(\pmb{w}-\pmb{w}^*)
$$

Where $\pmb{H}$ denotes the Hessian, the $D\times D$ symmetric matrix with entries:

$$
\pmb{H}_{i, j} = \frac{\partial^2\cost{\pmb{w}}}{\partial w_i \partial w_j}
$$
...
$$
\pmb{x}_n \sigma(\pmb{x}_n^T\pmb{w}) - y_n
$$

Each term only depends on $\pmb{w}$ in the $\sigma(\pmb{x}_n^T w)$ term. Therefore, the Hessian associated to one term is:
...
\begin{align}
\pmb{H}(\pmb{w}) 
    & = \sum_{n=1}^N{\nabla^2\mathcal{L}_n(\pmb{w})} \\
    & = \sum_{n=1}^N{
        \underbrace{\pmb{x}_n \pmb{x}_n^T}_{D\times D}
        \sigma(\pmb{x}_n^T \pmb{w})
        \bigl(1 - \sigma(\pmb{x}_n^T \pmb{w}) \bigr)
    } \\
    & = \underbrace{\ \pmb{X}^T \ }_{D\times N} \ 
        \underbrace{\ \pmb{S}   \ }_{N\times N} \ 
        \underbrace{\ \pmb{X}   \ }_{N\times D} \\
\end{align}
$$

The $\pmb{S}$ matrix is diagonal, where:

$$
S_{n, n} = \sigma(\pmb{x}_n^T \pmb{w})\bigl(1 - \sigma(\pmb{x}_n^T \pmb{w}) \bigr)
$$
...
$$
\pmb{w}^{(t+1)} = \pmb{w}^{(t)} - \pmb{H}(\pmb{w}^{(t)})^{-1} \nabla\cost{\pmb{w}^{(t)}} \gamma^{(t)}
$$
...
$$
y = x^T \pmb{w} + z, \quad \text{with } z\sim\normal{0, \sigma^2}
$$
...
4. $\mu := \expect{\pmb{\phi}(y)}$

Proofs for the first 3 properties are in the lecture notes. The last property is given without proof.

### Application in ML

We use $\pmb{\eta} = \pmb{x}^T\pmb{w}$.

#### Maximum Likelihood Parameter Estimation
Assume that we have samples composing our training set, $\Strain = \set{(x_n, y_n)}_{n=1}^N$ i.i.d. from some distribution, which we assume is some exponential family. Assume we have picked a model, i.e. that we fixed $h(y)$ and $\pmb{\phi}(y)$, but that $\pmb{\eta}$ is unknown. How can we find an optimal $\pmb{\eta}$?
...
\mathcal{L}_{LL}(\pmb{\eta})
    & = -\log{(p(y \mid \pmb{\eta}))} \\
    & = \sum_{n=1}^N{\left(
        -\log{\left[h(y_n)\right] - \pmb{\eta}^T\pmb{\phi}(y_n) + A(\pmb{\eta})}
    \right)}
\end{align}
$$
...
For such a model, the maximum likelihood problem, as described above, is easy to solve. As we've noted above, the cost function is convex, so a greedy, iterative algorithm should work well. Let's look at the gradient of the cost in terms of $\pmb{w}$ (instead of $\pmb{\eta} = \pmb{x}^T\pmb{w}$ as previously):

$$
\begin{align}
\cost{\pmb{w}} 
    & = -\sum_{n=1}^N{
        \log{(h(y_n))} + \pmb{x}_n^T\pmb{w} \pmb{\phi}(y_n) - A(\pmb{x}_n^T\pmb{w})
    } \\

\nabla_{\pmb{w}}\cost{\pmb{w}} 
    & = -\sum_{n=1}^N{
        \pmb{x}_n \pmb{\phi}(y_n) - \nabla_{\pmb{w}} A(\pmb{x}_n^T\pmb{w})
    }
\end{align}
$$
...
$$
\nabla_{\pmb{w}}\cost{\pmb{w}} = - \sum_{n=1}^N {\pmb{x}_n \pmb{\phi}(y_n) - \pmb{x}_n g^{-1}(\pmb{x}_n^T\pmb{w})}
$$

Setting this to zero gives us the condition of optimality. Using matrix notation, we can rewrite this sum as follows:

$$
\nabla\cost{\pmb{w}} = \pmb{X}^T\left( g^{-1}(\pmb{Xw}) - \pmb{\phi}(\pmb{y}) \right) = 0
$$
...
For simplicity, let's assume that we're operating in a d-dimensional box, that is, in the domain $\chi = [0, 1]^d$. As always, we have a training set $\Strain=\set{(\pmb{x}_n, y_n)}$. 

### K Nearest Neighbor (KNN)
Given a "fresh" input $\pmb{x}$, we can make a prediction using $\text{nbh}\_{\Strain,\ k}(\pmb{x})$. This is a set of the $k$ inputs in the training set that are closest to $\pmb{x}$.

For the regression problem, we can take the average of the k nearest neighbors:

$$
f(\pmb{x}) = \frac{1}{k}\sum_{n\in\text{nbh}_{\Strain,\ k}(\pmb{x})}{y_n}
$$
...
$$
f(\pmb{x}) = \text{maj}\set{y_n : n \in \text{nbh}_{\Strain, k}(\pmb{x})}
$$
...
On the right-hand side we have Euclidean distance. In other words, we ask that the conditional probability $\mathbb{P}\left\\{y=1\mid\pmb{x}\right\\}$, denoted by $\eta(x)$, be [Lipschitz continuous](https://en.wikipedia.org/wiki/Lipschitz_continuity) with Lipschitz constant $\mathcal{c}$. We will use this assumption later on to prove a performance bound for our KNN model.
...
The second to last step uses the fact that $\eta$ is a probability distribution, so $-1 \le 2\eta(\pmb{x})-1 \le 1$. The last step uses the $\ref{eq:lipschitz-bound}$.
...
In the figure above, the pink region represents the "margin" created by the SVM. The center of the margin is the separating hyperplane; its direction is perpendicular to $\pmb{w}$, the normal vector defining the hyperplane. The margin's total width is $2/\norm{\pmb{w}}$.

Points inside the margin are feature vectors $\pmb{x}$ for which $\abs{\pmb{x}^T\pmb{w}} < 1$. These points incur a cost with hinge loss. Any points outside the margin, for which $\abs{\pmb{x}^T\pmb{w}} \ge 1$, do not incur any cost, as long as they're on the correct side. Thus, depending on the $\pmb{w}$ that we choose, the orientation and size of the margin will change; there will be a different number of points in it, and the cost will change.
...
{% details Proof %}
We can prove this formulation by using the following identity. If we let $\pmb{P}$ be an $N\times M$ matrix, and $\pmb{Q}$ be $M\times N$. Then:

$$
\pmb{P}(\pmb{QP} + \pmb{I}_M) = \pmb{PQP} + \pmb{P} = (\pmb{PQ} + \pmb{I}_N)\pmb{P}
$$

Assuming that $(\pmb{QP} + \pmb{I}_M)$ and $(\pmb{PQ} + \pmb{I}_N)$ are invertible, we have the identity:

$$
(\pmb{PQ}+\pmb{I}_N)^{-1}\pmb{P} = \pmb{P}(\pmb{QP}+\pmb{I}_M)^{-1}
$$

To derive the formula, we can let $\pmb{P} = \pmb{X}^T$ and $\pmb{Q} = \frac{1}{\lambda}\pmb{X}$.
{% enddetails %}

#### Representer theorem
The representer theorem generalizes what we just saw about ridge regression. For a $\pmb{w}^*$ minimizing the following, for any cost $\mathcal{L}_n$,
...
\pmb{\alpha}^* = \argmin_{\pmb{\alpha}}{
    \frac{1}{2}\pmb{\alpha}^T(\pmb{XX}^T + \lambda \pmb{I}_N)\pmb{\alpha} 
    - \pmb{\alpha}^T \pmb{y}}
$$
...
= (\pmb{XX}^T + \lambda \pmb{I}_N)\pmb{\alpha} - \pmb{y} = 0 
$$
...
$$
\begin{align}
\pmb{\alpha}^*  & = (\pmb{XX}^T + \lambda \pmb{I}_N)^{-1} \pmb{y} \\
\pmb{w}^*       & = \pmb{X}^T\pmb{\alpha}^* 
                  = \pmb{X}^T(\pmb{XX}^T + \lambda \pmb{I}_N)^{-1} \pmb{y}
\end{align}
$$
...
#### Kernel functions
The kernel is defined as $\pmb{K} = \pmb{XX}^T$. We'll call this the **linear kernel**. The elements are defined as:

$$
\pmb{K} = \pmb{XX}^T = \begin{bmatrix}
\pmb{x}_1^T\pmb{x}_1 & \pmb{x}_1^T\pmb{x}_2 & \cdots & \pmb{x}_1^T\pmb{x}_N \\
\pmb{x}_2^T\pmb{x}_1 & \pmb{x}_2^T\pmb{x}_2 & \cdots & \pmb{x}_2^T\pmb{x}_N \\
\vdots               & \vdots               & \ddots & \vdots               \\
\pmb{x}_N^T\pmb{x}_1 & \pmb{x}_N^T\pmb{x}_2 & \cdots & \pmb{x}_N^T\pmb{x}_N \\  
\end{bmatrix}
$$

The kernel matrix is a $N\times N$ matrix. Now, assume that we had first augmented the feature space with $\phi(\pmb{x})$; the elements of the kernel would then be:

$$
\pmb{K} = \pmb{\Phi}\pmb{\Phi}^T = \begin{bmatrix}
\phi(\pmb{x}_1)^T\phi(\pmb{x}_1) & \phi(\pmb{x}_1)^T\phi(\pmb{x}_2) & \cdots & \phi(\pmb{x}_1)^T\phi(\pmb{x}_N) \\
\phi(\pmb{x}_2)^T\phi(\pmb{x}_1) & \phi(\pmb{x}_2)^T\phi(\pmb{x}_2) & \cdots & \phi(\pmb{x}_2)^T\phi(\pmb{x}_N) \\
\vdots & \vdots & \ddots & \vdots \\
\phi(\pmb{x}_N)^T\phi(\pmb{x}_1) & \phi(\pmb{x}_N)^T\phi(\pmb{x}_2) & \cdots & \phi(\pmb{x}_N)^T\phi(\pmb{x}_N) \\
\end{bmatrix}
$$

Using this formulation allows us to keep the size of $\pmb{K}$ the same, regardless of how much we augment. In other words, we can now solve a problem where the size is independent of the feature space.

The feature augmentation goes from $\pmb{x}_n \in \mathbb{R}^D$ to $\phi(\pmb{x}_n) \in \mathbb{R}^{D'}$ with $D' \gg D$, or even to an infinite dimension.
...
$$
\kappa(\pmb{x}_1, \pmb{x}_2) = \pmb{x}_1^T\pmb{x}_2 \implies \phi(\pmb{x}) = \pmb{x}
$$
...
##### Polynomial kernel 
Let's assume that $\pmb{x}', \pmb{x}' \in\mathbb{R}^3$. Let's define the kernel function as follows:
...
What is the $\phi$ corresponding to this? The inner product that would produce the above would is produced by taking the inner product $\phi(\pmb{x})^T\phi(\pmb{x}')$, where $\phi$ is defined as follows:

$$
\phi(\pmb{x}) = \begin{bmatrix}
\sqrt{2} x_1 x_2 &
\sqrt{2} x_1 x_3 &
\sqrt{2} x_3 x_3 &
x_1^2 &
...
$$
\kappa(\pmb{x}, \pmb{x}') = \exp{\left[-(\pmb{x} - \pmb{x}')^T(\pmb{x} - \pmb{x}')\right]}
$$

This is called the *radial basis function* (RBF) kernel.

Consider the special case in which $\pmb{x}$ and $\pmb{x}'$ are scalars; we'll look at the Taylor expansion of the function:
...
\begin{align}
\kappa(\pmb{x}, \pmb{x'})
     & = a\kappa_1(\pmb{x}, \pmb{x'}) + b\kappa_2(\pmb{x}, \pmb{x'}),
     & \quad \forall a, b \ge 0 \\

\kappa(\pmb{x}, \pmb{x'})
    & = \kappa_1(\pmb{x}, \pmb{x'}) \kappa_2(\pmb{x}, \pmb{x'}) \\

\kappa(\pmb{x}, \pmb{x'}) 
    & = \kappa_1(f(\pmb{x}), f(\pmb{x'})),
    & \quad f: \mathbb{R}^D \rightarrow \mathbb{R}^D \\

\kappa(\pmb{x}, \pmb{x}') 
    & = f(\pmb{x})f(\pmb{x}'),
    & \text{in which case } \phi(\pmb{x}) = f(\pmb{x}) \\
\end{align}
$$
...
Recall that the classifier predicts $y = \phi(\pmb{x})^T\pmb{w}^\*$, and that $\pmb{w}^\* = \pmb{X}^T \pmb{\alpha}^\*$. This leads us to:

$$
y = \phi(\pmb{x})^T \phi(\pmb{X})^T \pmb{\alpha} 
  = \sum_{n=1}^N{\kappa(\pmb{x}, \pmb{x_n})\pmb{\alpha}}
$$

### Properties of kernels
How can we ensure that there exists a feature augmentation $\phi$ corresponding to a given kernel $\pmb{K}$? A kernel function must be an inner-product in some feature space. Mercer's condition states that we have this iff the following conditions are fulfilled:

1. $K$ is symmetric, i.e. $\kappa(\pmb{x}, \pmb{x'}) = \kappa(\pmb{x'}, \pmb{x})$
2. For any arbitrary input set $\set{\pmb{x}_n}$ and all $N$, $K$ is positive semi-definite
...
$$
\min_{\pmb{\mu}, \pmb{z}}{\mathcal{L}_{\text{K-means}}(\pmb{z}, \pmb{\mu})}
= \min_{\set{\pmb{\mu}_k}, \set{z_{nk}}}{
    \sum_{n=1}^N{\sum_{k=1}^K{
        z_{nk} \norm{\pmb{x}_n - \pmb{\mu}_k}^2
    }}
}
$$

The $z_{nk}$ is the k<sup>th</sup> number in the $\pmb{z}_n$ vector, which is a one-hot vector encoding the cluster assignment. Every datapoint $\pmb{x}_n$ has an associated vector $\pmb{z}_n$ of length K, that takes value 1 in the index of the cluster to which $\pmb{x}_n$ belongs, and 0 everywhere else. Mathematically, we can write this constraint as:
...
$$
\begin{align}
\pmb{z}_n & = \left[z_{n1}, z_{n2}, \dots, z_{nK}  \right]^T \\
\pmb{z}   & = \left[\pmb{z}_1, \pmb{z}_2, \dots, \pmb{z}_N\right]^T \\ 
...
$$
\pmb{\mu}_k = \frac{\sum_{n=1}^N{z_{nk} \pmb{x}_n}}{\sum_{n=1}^N{z_{nk}}}
$$
...
= \sum_{n=1}^N{2 z_{nk} \pmb{\mu}_k - 2 z_{nk} \pmb{x}_n}
= 0 
$$
...
2. Repeat until convergence:
    1. Choose $\pmb{z}^{(t+1)}$ given $\pmb{\mu}^{(t)}$
    2. Choose $\pmb{\mu}^{(t+1)}$ given $\pmb{z}^{(t+1)}$
...
There are other ways to look at K-means. One way is to think of it as a coordinate descent, minimizing a cost function by finding parameters $\pmb{\mu}$ and $\pmb{z}$ iteratively:

$$
\begin{align}
\pmb{z}^{(t+1)}   & = \argmin_{\pmb{z}} \cost{\pmb{z}, \pmb{\mu}^{(t)}} \\
\pmb{\mu}^{(t+1)} & = \argmin_{\pmb{\mu}} \cost{\pmb{z}^{(t+1)}, \pmb{\mu}}
\end{align}
$$
...
$$
\min_{\pmb{\mu}, \pmb{z}}{\mathcal{L}_{\text{K-means}}(\pmb{z}, \pmb{\mu})} 
= \min_{\pmb{M}, \pmb{Z}}{\frobnorm{\pmb{X}^T - \pmb{M} \pmb{Z}^T}}
$$

A few notes on this notation:

- $\pmb{X}$ is, as always, the $N\times D$ data matrix
- $\pmb{M}$ is a $D\times K$ matrix representing the mean, the $\pmb{\mu}_k$ vectors; each column represents a different center
- $\pmb{Z}^T$ is the $K\times N$ assignment matrix, the $\pmb{z}_n$ vectors; it picks a single column from the means. This means that the columns of $\pmb{Z}$ are one-hot vectors, i.e. that exactly one element of each column of $\pmb{Z}^T$ is 1
- $\pmb{X}^T - \pmb{M} \pmb{Z}^T$ computes a matrix whose rows are vectors from each point to its corresponding cluster center.
...
This is indeed a matrix factorization as we're trying to find two matrices $\pmb{M}$ and $\pmb{Z}$ that minimize the above criterion.
...
The assumption is that we have generated the data by using $K$ separate $D$-dimensional Gaussian distributions. Each sample $\pmb{x}_n$ comes from one of the $K$ distributions uniformly at random. These distributions are assumed to have means $\set{\pmb{\mu}_k}$, and the identity matrix as their covariance matrix (that is, variance 1 in each dimension, and the dimensions are i.i.d).

Let's write down the likelihood of a sample $\pmb{x}_n$. It's the Gaussian density function of the cluster to which the sample belongs:

$$
p(\pmb{x}_n \mid \pmb{\mu}, \pmb{z}) = \prod_{k=1}^K{\left(
    \frac{1}{(2\pi)^{D/2}} \exp{\frac{-\norm{\pmb{x}_n - \pmb{\mu}_k}^2}{2}}
\right)^{z_{nk}}}
$$

The density assuming that we know that the points are from a given $k$ is what's inside of the large parentheses. We use $z_{nk}$ in the exponent to cancel out the contributions of the clusters to which $\pmb{x}_n$ does not belong, keeping only the contribution of its cluster.
...
$$
p(\pmb{X}\mid\pmb{\mu},\pmb{z}) =
\prod_{n=1}^N{p(\pmb{x}_n \mid \pmb{\mu}, \pmb{z})} = \prod_{n=1}^N{\prod_{k=1}^K{\left(
    \frac{1}{(2\pi)^{D/2}} \exp{\frac{-\norm{\pmb{x}_n - \pmb{\mu}_k}^2}{2}}
\right)^{z_{nk}}}}
$$
...
$$
\begin{align}
-\log{p(\pmb{X}\mid\pmb{\mu},\pmb{z})} 
    & = -\log{\prod_{n=1}^N{p(\pmb{x}_n \mid \pmb{\mu}, \pmb{z})}} \\
...
#### Soft clustering
Another extension is that we were previously forced to be either from one or another distribution. This is called hard clustering. We can generalize this to soft clustering, where a point can be associated to multiple clusters. In soft clustering, we model $\pmb{z}_n$ as a random variable taking values in $\set{1, \dots, K}$, instead of a one-hot vector. 
...
$$
\begin{align}
p(\pmb{X}, \pmb{z} \mid \pmb{\mu}, \pmb{\Sigma}, \pmb{\pi}) 
    & = \prod_{n=1}^N{p(z_n \mid \pmb{\pi}) \normal{\pmb{x}_n \mid z_n, \pmb{\mu}, \pmb{\Sigma}}} \\
    & = \prod_{n=1}^N{
        \prod_{k=1}^K{\left(\pi_k \normal{\pmb{x}_n\mid\pmb{\mu}_k, \pmb{\Sigma}_k}\right)^{z_{nk}}}
    } \\
\end{align}
$$
...
The $\pmb{z}_n$ variable is what's known asf a **latent variable**; it's not something that we observe directly, it's just something that we use to make our model more complex. The parameters of the model are $\pmb{\theta} := \set{\pmb{\mu}, \pmb{\Sigma}, \pmb{\pi}}$.
...
$$
p(\pmb{X}\mid\pmb{\theta}) = 
\prod_{n=1}^N{p(\pmb{x}_n \mid \pmb{\theta})} =
\prod_{n=1}^N{\sum_{k=1}^K{\pi_k \normal{\pmb{x}_n \mid \pmb{\mu}_k, \pmb{\Sigma}_k}}}
$$
...
Before, the number of parameters in the model was $\mathcal{O}(N)$; having marginalized out the latent variables, the growth is reduced $\mathcal{O}(D^2 K)$ (which is much better, assuming that $D, K \ll N$).
...
\cost{\pmb{\theta}^{(t)}} = 
\sum_{n=1}^N{\log{\left(
    \sum_{k=1}^K{\pi_k^{(t)} \normal{\pmb{x}_n \mid \pmb{\mu}_k^{(t)}, \pmb{\Sigma}_k^{(t)}}}
\right)}}
$$
...
\sum_{n=1}^N{\sum_{k=1}^K}{q_{nk}^{(t+1)} \log{\left(
    \frac{\pi_k \normal{\pmb{x}_n \mid \pmb{\mu}_k, \pmb{\Sigma}_k}}{q_{nk}^{(t+1)}}
\right)}}
$$
...
$$
\begin{align}
\log{\left( \sum_{k=1}^K{\pi_k \normal{\pmb{x}_n \mid \pmb{\mu}_k, \pmb{\Sigma}_k}} \right)} 
    & = \log{\left(
        \sum_{k=1}^K{
            q_{nk}^{(t)}
            \frac{
...
$$
\begin{align}
\pmb{\mu}_k^{(t+1)} & := \frac{\sum_n{q_{nk}^{(t)} \pmb{x}_n}}{\sum_n{q_{nk}^{(t)}}} \\ \\

\pmb{\Sigma}_k^{(t+1)} & := \frac{
    \sum_n{q_{nk}^{(t)} (\pmb{x}_n - \pmb{\mu}_k^{(t+1)}) (\pmb{x}_n - \pmb{\mu}_k^{(t+1)})^T}
}{
    \sum_n{q_{nk}^{(t)}}
} \\ \\
...
$$
\begin{align}
p(\pmb{x}_n \mid \pmb{\theta}) 
& = \sum_{k=1}^K {p(\pmb{x}_n, z_n = k \mid \pmb{\theta})}
  = \sum_{k=1}^K {p(z_n = k \mid \pmb{\theta}) p(\pmb{x}_n \mid z_n = k, \pmb{\theta})} \\ 
& = \sum_{k=1}^K {\pi_k \normal{\pmb{x}_n \mid \pmb{\mu}_k, \pmb{\Sigma}_k}}
\end{align}
$$

Note that we can generalize the EM algorithm to other choices of $p(\pmb{x}_n, z_n = k \mid \pmb{\theta})$, but that this is the one we used here.

This probability is an expectation based on the prior $\pi_k$. Let's now look at the posterior distribution of $z_n$, given the datapoints $\pmb{x}_n$:

$$
\begin{align}
p(z_n = k \mid \pmb{x}_n, \pmb{\theta})
    & = \frac{p(z_n = k, \pmb{x}_n, \pmb{\theta})}
             {p(\pmb{x}_n, \pmb{\theta})}
      = \frac{p(z_n = k, \pmb{x}_n \mid \pmb{\theta})}
             {p(\pmb{x}_n \mid \pmb{\theta})} \\
    & = \frac{p(z_n = k, \mid \pmb{\theta})p(\pmb{x}_n \mid z_n = k, \pmb{\theta})}
             {p(\pmb{x}_n \mid \pmb{\theta})} \\ 
    & = \frac{p(z_n = k, \mid \pmb{\theta})p(\pmb{x}_n \mid z_n = k, \pmb{\theta})}
             {\sum_{j=1}^K p(z_n = j\mid\pmb{\theta})p(\pmb{x}_n\mid z_n = j, \pmb{\theta})} \\ 
    & = \frac{\pi_k \normal{\pmb{x} \mid \mu_k, \pmb{\Sigma}_k}}
             {\sum_{j=1}^K{\pi_j \normal{\pmb{x} \mid \mu_j, \pmb{\Sigma}_j}}} =: q_{nk}
\end{align}
$$

The distribution that we previously just explained as an abstract, unknown distribution is in fact the posterior $p(z_n = k \mid \pmb{x}_n, \pmb{\theta})$.

We can now explain why the E step is the *expectation* step. Assume that we know the $q_{nk}$ (as a thought experiment, imagine a genie told us the assignment probabilities of each sample $\pmb{x}\_n$ to a component $k$, which is exactly what the $q_{nk}$ quantities are). 

As a reminder, the log-likelihood is:

$$
\begin{align}
\log{p(\pmb{x}_n, z_n = k \mid \pmb{\theta})} 
& = \log{\left(
    p(z_n = k \mid \pmb{\theta}) p(\pmb{x}_n \mid \pmb{\mu}_k, \pmb{\Sigma}_k)
\right)} \\
& = \log{\left(
    \pi_k \normal{\pmb{x}_n \mid \pmb{\mu}_k, \pmb{\Sigma}_k}
\right)}
...
$$
\expectsub{z_n}{\log{p(\pmb{x}_n, z_n = k \mid \pmb{\theta})}} =
\sum_{k=1}^K{q_{nk} \log{\pi_k \normal{\pmb{x}_n \mid \pmb{\mu}_k, \pmb{\Sigma}_k}}}
$$

Summing this over all samples $\pmb{x}_n$, we find the cost
...
$$
\pmb{\theta}^{(t+1)} = \argmax_{\pmb{\theta}}{\expectsub{p\left(z_n \mid \pmb{x}_n, \pmb{\theta}^{(t)}\right)}{\log{p(\pmb{x}_n, z_n \mid \pmb{\theta})}}}
$$
...
$$
\pmb{X} \approx \pmb{W}\pmb{Z}^T
$$

The hope is to "explain" each rating $x_{dn}$ by a numerical representation of the corresponding movie and user.

Here, we have a "tall" matrix $W\in\mathbb{R}^{D\times K}$, and $\pmb{Z}\in\mathbb{R}^{N\times K}$, forming a "flat matrix" $\pmb{Z}^T \in \mathbb{R}^{K\times N}$. In practice, compared to the size of $N$ or $D$, $K$ will be relatively small (maybe 50 or so).

We'll assign a cost function that we're trying to optimize:

$$
\min_{\pmb{W}, \pmb{Z}} \cost{\pmb{W}, \pmb{Z}} 
:= \min_{\pmb{W}, \pmb{Z}} \frac{1}{2} \sum_{(d, n)\in\Omega}{\left[
    x_{dn} - (\pmb{WZ}^T)_{dn}
\right]^2}
$$
...
\frac{1}{2} \sum_{(d, n)\in\Omega}{\left[
    x_{dn} - (\pmb{WZ}^T)_{dn}
\right]^2} 
    + \frac{\lambda_w}{2}\frobnorm{\pmb{W}}^2
    + \frac{\lambda_z}{2}\frobnorm{\pmb{Z}}^2
$$
...
$$
\cost{\pmb{W}, \pmb{Z}} =
\sum_{(d, n)\in\Omega}{f_{d, n}(\pmb{W}, \pmb{Z})}
= \sum_{(d, n)\in\Omega}{\frac{1}{2}\left[
    x_{dn} - (\pmb{WZ}^T)_{dn}
\right]^2}
$$

Let's derive the stochastic gradient for an individual loss function (which is what we need to compute when doing SGD). Matrix calculus is not easy, but understanding it starts with understanding the following sentence: *a gradient with respect to a matrix is a matrix of gradients*. If we compute the gradient of a function $f$ with respect to a matrix $\pmb{X}\in\mathbb{R}^{D\times N}$, we get a gradient matrix $\pmb{g}\in\mathbb{R}^{D\times N}$, where each element $g_{a, b}$ is the derivative of $f$ with respect to the $(a, b)$ element of $\pmb{X}$:
...
$$
\begin{align}
\nabla_{\pmb{W}}  f_{d, n} & \in \mathbb{R}^{D\times K} \\
\nabla_{\pmb{Z}}  f_{d, n} & \in \mathbb{R}^{N\times K}
\end{align}
$$

Luckily, we're not doing the full gradient here, but only the *stochastic* gradient, which only requires computing a single entry in the gradient matrix. Therefore, for a fixed pair $(d, n)$ (that is, a rating from user $n$ of movie $d$), we will compute a single entry $(d', k)$ in the $\pmb{W}$ derivative:

$$
\left(\nabla_{\pmb{W}} f_{d, n}\right)_{(d', k)}
= \frac{\partial f_{d, n}}{\partial w_{d', k}}(\pmb{W}, \pmb{Z})
= \begin{cases}
...
The same goes for the derivative by $\pmb{Z}$. We'll compute a single entry $(n', k)$ in $\nabla_{\pmb{W}} f_{d, n}$:

$$
\left(\nabla_{\pmb{Z}} f_{d, n}\right)_{(n', k)}
= \frac{\partial f_{d, n}}{\partial z_{n', k}}(\pmb{W}, \pmb{Z})
= \begin{cases}
...
$$
\pmb{W}^{(t+1)} = \pmb{W}^{(t)} - \gamma \nabla_{\pmb{W}} f_{d, n} \\
\pmb{Z}^{(t+1)} = \pmb{Z}^{(t)} - \gamma \nabla_{\pmb{Z}} f_{d, n} \\
$$

With stochastic gradient descent, we only compute the gradient of a single $f_{d, n}$ instead of the whole cost $\mathcal{L}$. Therefore, each step only updates the d<sup>th</sup> row of $\pmb{W}$, and the n<sup>th</sup> row of $\pmb{Z}$.

### Alternating least squares (ALS)
The alternating minimization algorithm alternates between optimizing $\pmb{Z}$ and $\pmb{W}$. ALS is a special case of this, with square error.
...
& = \frac{1}{2}\frobnorm{\pmb{X} - \pmb{WZ}^T}^2 
    + \frac{\lambda_w}{2} \frobnorm{\pmb{W}}^2 
    + \frac{\lambda_z}{2} \frobnorm{\pmb{Z}}^2
\end{align}
$$
...
These are two least squares problems. The only difference is that we're searching for a whole matrix in this case, unlike in least squares where we searched for a vector. Still, we can find a closed form for it by setting the gradient with respect to $\pmb{W}$ and then $\pmb{Z}$ to 0, which will give:

$$
\begin{align}
(\pmb{Z}^*)^T & := (\pmb{W}^T \pmb{W} + \lambda_z \pmb{I}_K)^{-1} \pmb{W}^T \pmb{X} \\
(\pmb{W}^*)^T & := (\pmb{Z}^T \pmb{Z} + \lambda_w \pmb{I}_K)^{-1} \pmb{Z}^T \pmb{X}^T \\
\end{align}
$$
...
To attempt to get the meaning of words, we can start by constructing co-occurrence counts from a big corpus or text. This is a matrix $\pmb{N}$ in which $n_{ij}$ is the number of contexts where word $w_i$ occurs together with word $w_j$. A context is a window of words occurring together (it could be a document, paragraph, sentence, or a window of $n$ words). 
...
It does so by computing the inner product score of the words; $\pmb{w}\_d^T \pmb{w}\_n$ is real, and must be distinguished from the fake $\pmb{w}\_d^T \pmb{w}\_{n'}$.
...
To construct a word embedding, we want to find a factorization of the co-occurrence matrix $\pmb{N}$. Typically, we actually use $\pmb{X} = \log{\pmb{N}}$ as the element-wise log of the co-occurrence matrix, i.e. $x_{dn} := \log{(n_{dn})}$. We'll find a factorization such that:

$$
\pmb{X} \approx \pmb{W}\pmb{Z}^T
$$

As before, we let $\Omega\subseteq [D] \times [N]$ collect the indices of non-zero counts in $\pmb{X}$. In other words, $\Omega$ contains indices of word pairs that have been observed in the same context.

For each pair of observed words $(w_d, w_n) \in \Omega$, we'll try to explain their co-occurrence count by a numerical representation of the two words; the d<sup>th</sup> row of $\pmb{W}$ is the representation of a word $w_d$, and n<sup>th</sup> row of $\pmb{Z}$ is the representation of a context word $w_n$.

To do this, we do the following cost minimization

$$
\min_{\pmb{W}, \pmb{Z}} \cost{\pmb{W}, \pmb{Z}}
:= \frac{1}{2} \sum_{(d, n)\in\Omega} f_{dn} \left( x_{dn} - (\pmb{W}\pmb{Z}^T)_{dn} \right)^2
$$
...
In our subsequent discussion, $\pmb{X}$ is the $D \times N$ data matrix, whose $N$ columns represent the feature vectors in $D$-dimensional space. 

The PCA will be computed from the data matrix $\pmb{X}$ using singular value decomposition.

### SVD
The **singular value decomposition** (SVD) of a $D \times N$ matrix $\pmb{X}$ is:
...
- $\pmb{V}$ is a $N \times N$ orthonormal matrix
...
We'll assume $D < N$ without loss of generality (we could just take the transpose of $\pmb{X}$ otherwise). This is an arbitrary choice, but helps us tell the dimensions apart.

The diagonal entries in $\pmb{S}$ are the *singular values* in descending order: 

$$
s_1 \ge s_2 \ge \dots \ge s_D \ge 0
$$

The columns of $\pmb{U}$ and $\pmb{V}$ are the *left* and *right singular vectors*.
...
**Lemma**: For any $D \times N$ matrix $\pmb{X}$ and any $D\times N$ rank-K matrix $\hat{\pmb{X}}$:
...
- $\pmb{X} = \pmb{U}\pmb{S}\pmb{V}^T$ is the SVD of $\pmb{X}$
- $s_i$ are the singular values of $\pmb{X}$
- $\pmb{U}_K$ is the $D\times K$ matrix of the first $K$ rows of $\pmb{U}$

***

If we use $\pmb{C} = \pmb{U}_K^T$ as our compression matrix, and $\pmb{R} = \pmb{U}_K$ as the reconstruction matrix, we get a better (or equal) error than  any other choice of reconstruction $\hat{\pmb{X}}$. This tells us that the best compression to dimension $K$ is a projection onto the first $K$ columns of $\pmb{U}$, which are the first $K$ left singular vectors.
...
The term $\pmb{U}_K \pmb{U}_K^T \pmb{X}$ has another simple interpretation. Let $\pmb{S}^{(K)}$ be the $D\times N$ diagonal matrix corresponding to a truncated version of $\pmb{S}$. It is of the same size, but only has the $K$ first diagonal values of $\pmb{S}$, and is zero everywhere else. We claim that:

$$
\pmb{U}_K \pmb{U}_K^T \pmb{X} = \pmb{U}_K \pmb{U}_K^T \pmb{USV}^T = \pmb{US}^{(K)}\pmb{V}^T
$$

> 👉 It's okay to drop the $K$ subscript on the $\pmb{U}$ matrix because $\pmb{S}^{(K)}$ already takes care of selecting the first $K$ rows
...
Expressing $\pmb{X}$ as an SVD allows us to easily get a matrix factorization.

$$
\pmb{X} 
= \pmb{USV}^T 
= \underbrace{\pmb{U}}_{\pmb{W}} \underbrace{\pmb{SV}^T}_{\pmb{Z}^T} 
= \pmb{WZ}^T
$$
...
- Matrix factorization started with a $\pmb{X}$ with many missing entries; the idea was that the factorization should model the existing entries well, so that we can predict the missing values. This is not something that the SVD can do.
...
= \underbrace{\pmb{U}_K}_{\pmb{W}}
  \underbrace{\pmb{S}^{(K)}\pmb{V}^T}_{\pmb{Z}^T} 
= \pmb{W}\pmb{Z}^T 
$$


### PCA and decorrelation
Assume that we have $N$ $D$-dimensional points in a $D\times N$ matrix $\pmb{X}$. We can compute the empirical mean and covariance by:
...
The covariance matrix $\pmb{K}$ is a $D \times D$ rank-1 matrix. If our data is from i.i.d. samples then these empirical values will converge to the true values when $N \rightarrow \infty$.

Before we do PCA, we need to *center the data around the mean*. Let's assume our data matrix $\pmb{X}$ has been preprocessed as such. Using the SVD, we can rewrite the empirical covariance matrix as: 
...
PCA finds orthogonal axes centered at the mean, that represent the most variance, in decreasing order of variance. Starting with orthogonal axes, it finds the rotation $\pmb{U}^T$ so that the axes point in the direction of maximum variance. This can be seen in [this visual explanation of PCA](http://setosa.io/ev/principal-component-analysis/). 

With this intuition about PCA in mind, let's try to decompose the covariance again, but this time considering the transformed, compressed data $\tilde{\pmb{X}} = \pmb{U}_K^T\pmb{X}$. The empirical covariance of along this transformed axis is:
...
To compute the SVD of a matrix $\pmb{X}$, we must compute the matrices $\pmb{U}$ and $\pmb{S}$ Let's see how we can do this efficiently. 

Let's consider the $D\times D$ matrix $\pmb{XX}^T$. As before, since $\pmb{V}$ is orthogonal, we can use the SVD to get:

$$
\pmb{X}\pmb{X}^T 
= \pmb{USV}^T\pmb{VS}^T\pmb{U} 
= \pmb{USS}^T\pmb{U}^T 
= \pmb{U} \pmb{S}_D^2 \pmb{U}^T
$$
...
### Pitfalls of PCA
Unfortunately, PCA is no miracle cure. The SVD is not invariant under scalings of the features in the original matrix $\pmb{X}$. This is why it's so important to normalize features. But there are many ways of doing this, and the result of PCA is highly dependent on how we do this, and there is a large degree of arbitrariness. 
