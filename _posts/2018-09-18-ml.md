...
$$
\arg{\max_\pmb{w}{p(y, X \mid \pmb{w})}}
$$
...
$$
\arg{\max_{\pmb{w}}{p(\pmb{y}, \pmb{X} \mid \pmb{w})}} = \arg{\max_{\pmb{w}}{p(\pmb{y} \mid \pmb{X}, \pmb{w})}}
$$
...
The sum of convex functions is also convex, so we simply need to prove that the inner part of the sum is convex. It just turns out that taking the derivative of the inner part of the sum gives us the logistic function:

$$
\frac{\partial \log{(1+\exp{(x_n^T w)})}}{\partial x} = \sigma(x)
$$

Thus, the derivative of the whole cost function is:

$$
\nabla\cost{\pmb{w}} = \sum_{n=1}^N{\pmb{x}_n(\sigma(\pmb{x}_n^T\pmb{w}) - y_n)} = \pmb{X}^T(\sigma(\pmb{Xw}) - \pmb{y})
$$

todo convexity

### Gradient descent
We can do gradient descent on this cost function as we've always done:
...
We'll assume that the Taylor expansion denotes exactly the cost function. In other words, we're assuming strict equality $=$ instead of approximation $\approx$ as above. Note that this is only an assumption; it isn't strictly true, but it's a decent approximation. Where does this take minimum value? To know that, let's set the gradient of the right-hand side to zero. This yields:

$$
H(\pmb{w}^*)^{-1} \nabla\cost{\pmb{w}^*} = \pmb{w}^* - \pmb{w}
$$

If we reorganize to have $w$ alone on one side of the equality, this gives us an iterative algorithm for finding the optimum:
