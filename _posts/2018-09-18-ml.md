...
$$
L_\mathcal{D}(f) = \mathbb{E}_\mathcal{D} [\mathcal{l}(y, f(\pmb{x}))]
$$
...
= \mathbb{E}_{S_\text{test}\sim\mathcal{D}}\left[ 
...
$$
\mathbb{E}[\Theta_n] = \mathbb{E}[\mathcal{l}(y_n, f(\pmb{x}_n))] = L_\mathcal{D}(f)
$$
...
**Chernoff Bound**: Let $\Theta_1, \dots, \Theta_n$ be a sequence of i.i.d random variables with mean $\mathbb{E}[\Theta]$ and range $[a, b]$. Then, for any $\epsilon > 0$:

$$
\mathbb{P}\left[
    \abs{\frac{1}{N}\sum_{n=1}^N {\Theta_n - \mathbb{E}[\Theta]}}
    \ge
    \epsilon
\right]
\le
