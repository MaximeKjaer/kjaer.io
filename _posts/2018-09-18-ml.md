...
$$
\pmb{w}^{(t+1)} = \pmb{w}^{(t)} - H(\pmb{w}^{(t)})^{-1} \nabla\cost{\pmb{w}^{(t)}} \gamma^{(t)}
$$

In this iterative algorithm, our starting point, $\pmb{w}^{(0)}$ corresponds to $\pmb{w}^*$.
...
$$
S_{n, n} = \sigma(x_n^T \pmb{w})\bigl(1 - \sigma(x_n^T \pmb{w}) \bigr)
$$
...
### Motivation
The motivation for this is that while logistic regression allows for, say, binary outputs, we may want to have something equivalently computationally efficient for, say, $y\in\mathbb{N}$. To do so, we introduce a different class of distributions, called the *exponential family*, with which we can revisit logistic regression and get other properties.
...
We're using $\pmb{\eta}$ as a shorthand for $\pmb{x}^T\pmb{w}$, and will do so in this section. More compactly, we can write this in a single formula:
...
p(y\mid\mu) 
    ...
    & = \exp{\left[
        ...
        \log{(1 - \mu)}
     \right]} \\
    & = \exp{\left[\pmb{\eta} \pmb{\phi}(y) - A(\eta)\right]}
\end{align}
$$
...
\begin{align}
\pmb{\phi}(y) & = y \\
\pmb{\eta}    & = \log{\frac{\mu}{1-\mu}} \\
A(\pmb{\eta}) & = -\log{(1-\mu)}=\log{(1 + e^{\pmb{\eta}})} \\
h(y)          & = 1
\end{align}
$$

Here, $\pmb{\phi}(y)$ is a scalar, which means that the family only depends on a single parameter. Note that $\pmb{\eta}$ and $\mu$ are linked:
...
\mu
= g^{-1}(\pmb{\eta})
= \log{\frac{e^{\pmb{\eta}}}{1+e^{\pmb{\eta}}}} 
= \sigma(\pmb{\eta})
$$
