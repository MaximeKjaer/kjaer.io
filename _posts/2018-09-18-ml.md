...
### Definition
Let's re-consider binary classification. In the following it will be more convenient to consider $y_n\in\left\\{\pm 1 \right\\}$. This is equivalent under the mapping $0 \mapsto -1$ and $1\mapsto 1$. This can be done continuously in the range by adding a constant feature 1 and scaling the weights by $\frac{1}{2}$.
...
Here, we use $z$ as shorthand for $\pmb{x}^T \pmb{w}$. This function is linear when it's below one, and does not punish predictions above one. This pushes us to give predictions that we can be very confident about (above one).
...
Let's assume $\lambda$ is small; we won't define that further, the main point is just that our priorities are as follows:
...
Previously, we computed $[z]_+$. This could be restated as the following:
...
There's a general theory on this topic (see [Nonlinear Programming](http://www.athenasc.com/nonlinbook.html) by Dimitri Bertsekas). In the case of SVMs though, the finding the function G is rather straightforward: 

$$
\min_{\pmb{w}}{\max_{\pmb{\alpha}\in[0, 1]^N}{\sum_{n=1}^N{\left[ 1 - y_n \pmb{x}_n^T \pmb{w}\right]_+} + \frac{\lambda}{2}\norm{\pmb{w}}^2}}
= \min_{\pmb{w}}{\max_{\pmb{\alpha}\in[0, 1]^N}{G(\pmb{w}, \pmb{\alpha})}}
$$
...
$$
\max_{\pmb{\alpha}\in[0, 1]^N}{\min_{\pmb{w}}{\sum_{n=1}^N{\left[ 1 - y_n \pmb{x}_n^T \pmb{w}\right]_+} + \frac{\lambda}{2}\norm{\pmb{w}}^2}}
$$
...
Where $\pmb{Y} := \text{diag}(\pmb{y})$.

If we plug this into the formulation of SVM, we get the following dual problem:

$$
\max_{\pmb{\alpha}\in[0, 1]^N}{\vec{1}^T\alpha - \frac{1}{2\lambda}\pmb{\alpha}^T\pmb{YXX}^T\pmb{Y\alpha}}
...
### K-Means
A common algorithm for unsupervised learning is called K-means (also called vector quantization in signal processing). The aim of this algorithm is to cluster the data: we want to find a partition such that every point is exactly one group, such that within a group, the (Euclidian) distance between points is much smaller than across the groups.
...
At every step, we try to go from a set of parameters $\theta^{(k)}$ to a better set of parameters $\theta^{(k+1)}$.
...
Due to the concavity of the log function, and because the $q_{nk}$ sum up to 1 (being a probability distribution), we can apply [Jensen's inequality](https://en.wikipedia.org/wiki/Jensen%27s_inequality) recursively to the sum to get:

$$
\begin{align}

\log{\left( \sum_{k=1}^K{\pi_k \mathcal{N}(x_n; \mu_k, \Sigma_k)} \right)} 
    & = \log{\left(
        \sum_{k=1}^K{
            q_{nk}^{(t)}
            \frac{
...
$$
q_{nk}^{(t)} \sim \pi_k \mathcal{N}(x_n; \mu_k, \Sigma_k)
$$
...
By now, we have an EM algorithm: we can optimize for $q_{nk}$ and $\theta$ alternatively. 

#### Expectation step
In the expectation step, we compute how well we're doing, by means of:

$$
\cost{\theta^{(t)}} = \sum_{n=1}^N{ \pi_k \mathcal{N}(x_n; \mu_k, \Sigma_k) }
$$
...
\cost{\theta^{(t+1)}} \ge \sum_{n=1}^N{\sum_{k=1}^K}{q_{nk}^{(t+1)} \log{\left(
    \frac{\pi_k \mathcal{N}(x_n; \mu_k, \Sigma_k)}{q_{nk}^{(t+1)}}
\right)}}
$$

In this problem, we're maximizing the cost function (instead of minimizing it as we're used to). This is strictly equivalent to minimizing the negative of this, and we're using maximizing and minimizing the negative equivalently. So getting a lower bound means that we have a monotonically non-decreasing cost over the steps $t$. Again, this is a good guarantee because we're maximizing over the cost: it tells us that our algorithm improves at every E-step.
...
{% details Proof %}
We now freeze the $q_n^{(t)}$ assignments, and optimize over $\theta$.
...
\sum_{n=1}^N{\sum_{k=1}^K}{
    q_{nk}^{(t)} \left[
        \log{\pi_k} - \log{q_{nk}^{(t)}} + \log{\mathcal{N}(x_n; \mu_k, \Sigma_k)}
    \right] + \lambda \sum_{k=1}^K{\pi_k}
}
$$

Differentiating with respect to $\pi_k$ gives us:
...
$$
\sum_{n=1}^N{\sum_{k=1}^K{q_{nk} \log{\pi_k \mathcal{N}(x_n; \mu_k, \Sigma_k)}}}
$$
