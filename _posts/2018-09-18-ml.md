...
[^mse-is-convex]: We accept this without a formal proof, but it should be clear from the [section on convexity](#convexity) that MSE is convex. Otherwise, the section on [normal equations for multi-parameter linear regression](#multiple-parameter-linear-regression) has more complete proofs.
...
But this idea fails miserably in high dimensions, where the geometry renders the idea of "closeness" meaningless. High-dimensional space is a very lonely place; in a high-dimensional space, if we grow the area around a point, we're likely to see no one for a very long time, and then once we get to the boundaries of the space, 💥, everyone is there at once. This is known as the [curse of dimensionality](https://en.wikipedia.org/wiki/Curse_of_dimensionality). The idea also fails when we have too little data, especially in high dimensions, where the closest point may actually be far away and a very bad indicator of the local situation.
...
#### Link function
As we've seen above, there is a relationship between the mean $\pmb{\mu} := \expect{\pmb{\phi}(y)}$ and $\pmb{\eta}$ using the link function $g$:
...
\argmin{\mathcal{L}_{LL}(\pmb{\eta})} 
    & = -\log{(p(y \mid \pmb{\eta}))} \\
    & = \sum_{n=1}^N{\left(
        -\log{\left[h(y_n) - \pmb{\eta}^T\pmb{\phi}(y_n) + A(\pmb{\eta})\right]}
    \right)}
\end{align}
$$
...
$$
\frac{1}{N} \nabla\cost{\pmb{\eta}}
    = -\left( \frac{1}{N} \sum_{n=1}^N{\pmb{\phi}(y)} \right)
      + \expect{\pmb{\phi}(y)} 
    = 0
$$
...
$$
\pmb{\eta} = g^{-1}\left( \frac{1}{N}\sum_{n=1}^N{\pmb{\phi}(y_n)} \right)
$$
...
$$
\nabla_{\pmb{w}}\cost{\pmb{w}} = - \sum_{n=1}^N {\pmb{x}_n \pmb{\phi}(y_n) - \nabla_{\pmb{w}} A(\pmb{x}_n^T\pmb{w})}
$$
...
Given a "fresh" input $\pmb{x}$, we can make a prediction using $\text{nbh}\_{S\_\text{train},\ k}(\pmb{x})$. This is a set of the $k$ inputs in the training set that are closest to $\pmb{x}$.
...
$$
f(\pmb{x}) = \text{maj}\left\{y_n : n \in \text{nbh}_{\Strain,\ k}(\pmb{x}) \right\}
$$
