...
Using this formulation allows to keep the size of $K$ the same, regardless of how much we augment. We'll see a few examples where we augment the feature space by a few dimensions, which of course isn't terribly useful (going to $N\times N$ just to be able to augment the feature space by 3 is rarely a good trade-off), but if we can augment our feature space to infinity and still keep our kernel finite (as we'll see later with RBF kernels), then the trade-off becomes worth it.
