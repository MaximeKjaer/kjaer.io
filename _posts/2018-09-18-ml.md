...
This function is linear when it's below one, and does not punish predictions above one. This pushes us to give predictions that we can be very confident about (above one).
...
$$
[z]_+ = \max{\left\{ 0, z \right\}} = \max{\alpha z}, \qquad \text{with } \alpha\in[0, 1]
$$
