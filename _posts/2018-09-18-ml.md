...
\nabla \mathcal{L}(\vec{w}) := \begin{bmatrix}
	\frac{\partial\mathcal{L}(\vec{w})}{\partial w_1} \\
	\vdots \\
	\frac{\partial\mathcal{L}(\vec{w})}{\partial w_D} \\
\end{bmatrix}
$$

This is a vector, i.e. $\nabla\mathcal{L}(\vec{w})\in\mathbb R^D$. Each dimension $i$ of the vector indicates how fast the cost $\mathcal{L}$ changes depending on the weight $w_i$.

#### Gradient descent

Gradient descent is an iterative algorithm. We start from a candidate $\vec{w}^{(t)}$, and iterate.De
...
When $\norm{\mathcal{L}(\vec{w})}$ is zero (or close to zero), we are often close to the optimum.
...
$$
\cost{\vec{w}^*} = 0, \quad \mathcal{L} \text{ convex} 
\implies 
...
$$
\vec{H}_{ij} = \frac{\partial^2\mathcal{L}}{\partial w_i \partial w_j}
$$
...
$$
\vec{H}(\vec{w}^*) := \frac{\partial^2 \cost{\vec{w}^*}}{\partial\vec{w}\partial\vec{w}^T} \text{ positive semidefinite}
\implies
...
$$
\mathcal{L}_{LL} := \log{p(\vec{y} \mid \vec{X}, \vec{w})}
...
To keep things in the following, we'll just claim that $\vec{X}^T\vec{X}$ is invertible. We'll also claim that the following set is an ellipsoid which scales around the origin as we change $\alpha$:
...
The $\vec{S}$ matrix is diagonal, where:
...
$$
\vec{w}^{(t+1)} = \vec{w}^{(t)} - \vec{H}(\vec{w}^{(t)})^{-1} \nabla\cost{\vec{w}^{(t)}} \gamma^{(t)}
$$
...
$$
\nabla_{\vec{w}}\cost{\vec{w}} = - \sum_{n=1}^N {\vec{x}_n \pmb{\phi}(y_n) - \vec{x}_n g^{-1}(\vec{x}_n^T\vec{w})}
$$

Setting this to zero gives us the condition of optimality. Using matrix notation, we can rewrite this sum as follows:

$$
\nabla\cost{\vec{w}} = \vec{X}^T\left( g^{-1}(\vec{Xw}) - \pmb{\phi}(\vec{y}) \right) = 0
$$
...
Recall that the classifier predicts $y = \phi(\vec{x})^T\vec{w}^\*$, and that $\vec{w}^\* = \vec{X}^T \pmb{\alpha}^\*$. This leads us to:

$$
y = \phi(\vec{x})^T \phi(\vec{X})^T \pmb{\alpha} 
  = \sum_{n=1}^N{\kappa(\vec{x}, \vec{x}_n)\pmb{\alpha}}
$$
...
$$
\min_{\pmb{\mu}, \vec{z}}{\mathcal{L}_{\text{K-means}}(\vec{z}, \pmb{\mu})} 
= \min_{\vec{M}, \vec{Z}}{\frobnorm{\vec{X}^T - \vec{M} \vec{Z}^T}}
$$
...
- $\vec{Z}^T$ is the $K\times N$ assignment matrix, the $\vec{z}_n$ vectors; it picks a single column from the means. This means that the columns of $\vec{Z}$ are one-hot vectors, i.e. that exactly one element of each column of $\vec{Z}^T$ is 1
...
\prod_{n=1}^N{p(\vec{x}_n \mid \pmb{\mu}, \vec{z})} = \prod_{n=1}^N{\prod_{k=1}^K{\left(
    \frac{1}{(2\pi)^{D/2}} \exp{\frac{-\norm{\vec{x}_n - \pmb{\mu}_k}^2}{2}}
\right)^{z_{nk}}}}
$$
...
#### Soft clustering
Another extension is that we were previously forced to be either from one or another distribution. This is called hard clustering. We can generalize this to soft clustering, where a point can be associated to multiple clusters. In soft clustering, we model $\vec{z}_n$ as a random variable taking values in $\set{1, \dots, K}$, instead of a one-hot vector. 
...
= \frac{\partial f_{d, n}}{\partial w_{d', k}}(\vec{W}, \vec{Z})
= \begin{cases}
...
= \frac{\partial f_{d, n}}{\partial z_{n', k}}(\vec{W}, \vec{Z})
= \begin{cases}
...
$$
\min_{\vec{W}, \vec{Z}} \cost{\vec{W}, \vec{Z}}
:= \frac{1}{2} \sum_{(d, n)\in\Omega} f_{dn} \left( x_{dn} - (\vec{W}\vec{Z}^T)_{dn} \right)^2
$$
...
$$
N \tilde{\vec{K}} = \tilde{\vec{X}} \tilde{\vec{X}}^T = \vec{U}^T\vec{X}\vec{X}^T\vec{U} = \vec{U}^T\vec{US}_D^2\vec{U}^T\vec{U} = \vec{S}_D^2
$$
...
A node outputs a non-linear function of a weighted sum of all the nodes in the previous layer, plus a bias term. For instance, the value of node $j$ at layer $l$ is given by: 
...
$$
f(x) = \max{\set{\vec{x}^T \vec{w}_1 + b_1, \dots, \vec{x}^T \vec{w}_k + b_k}}
$$
...
$$
f(\vec{x}^{(0)}) = f^{(L+1)} \circ \dots \circ f^{(2)} \circ f^{(1)}(\vec{x}^{(0)})
$$
...
& = \sum_k \delta_k^{(l+1)} \vec{W}_{i, j}^{(l+1)}
\end{align}
$$
...
$$
\delta^{(l)} = (\vec{W}^{(l+1)} \delta^{(l+1)}) \odot \phi'(\vec{z}^{(l)})
$$
...
Now that we have both $\vec{z}^{(l)}$ and $\delta^{(l)}$, let's go back to our initial goal, which is to compute the following:
...
### Regularization
Typically, we don't include bias terms in the regularization (experience shows that it just doesn't work quite as well). Therefore, the regularization term is expressed as something like:

$$
\Omega(\vec{W}) = \frac{1}{2} \sum_{i, j, l}{ \left( w_{i, j}^{(l)} \right)^2}
$$

In practice, there may be weights for the different layers, but we would still regularize with respect to the $L_2$ norm as above. This regularization favors small weights and can help us avoid overfitting. The gradient descent algorithm changes to the following:

$$
\pmb{\theta}^{(t+1)} = \pmb{\theta}^{(t)} - \lambda(\nabla_{\pmb{\theta}}\mathcal{L} + \pmb{\theta}^{(t)}) = \pmb{\theta}^{(t)} (1 - \lambda) + \lambda \nabla_{\pmb{\theta}}\mathcal{L}
$$

The weights in the update step are decreased by a factor $(1 - \lambda)$; we call this *weight decay*.

### Dataset augmentation
The more data we have, the better we can train. In some instances we can generate new data from the data we are given.

### Dropout
We pick some probability $p$, which is the probability of keeping a node or not in the network at a given step. This defines a different *subnetwork* at every step of SGD.

Dropout is a method to avoid overfitting (as nodes cannot "rely" on other nodes being present) and do model averaging (as there's an exponential number of subnetworks).

There are many variations of dropout; we talked about dropping nodes, but one could also drop edges.

### Convolutional nets
Classically, we've defined our networks as fully connected graphs. This means that if we have K nodes on both sides of the layer, we have $K^2$ edges (ergo weights) between them. Convolutional nets allow us to have somewhat more sparse networks.

Convolutions are common in signal processing, were we have a discrete-time system (that is, samples), which is denoted by $x^{(0)}[n]$. To process the stream we run it through a filter $f[n]$, which produces an output $x^{(1)}[n]$. This filter is often "local", looking at a window of size $k$ around a central value.

We have the same scenario if we think of a 2D picture, where the signal is $x^{(0)}[n, m]$. The filter can bring out various aspects, either smoothing features by averaging, or enhancing them by taking a high-pass filter.
...
To handle border cases, we can either do *zero padding*, where give the filter a default value when going over the edges. Otherwise, we can be careful only to run the filter within the bounds of the matrix, which is called *valid padding*.

#### Backpropagation
CNN uses weight sharing: many edges use weights that are the same. But this doesn't mean that we can't use backpropagation; we'll just forget that fact, considering that all weights are independent. Once we have the gradients, we sum up the gradients of all edges sharing the same weight, which gives us the gradient for this network with weight sharing.

### FFT
We've often seen in this course that there are multiple ways of thinking of the same things; for instance, we've often seen different models as variations of least squares.
