...
- Creating an actor returns its adress (not the Actor object)
- Adresses can be sent with messages (`sender`, for instance, which is captured automatically)

Actors are completely independent agents of computation:

- Local execution, no notion of global synchronization
- All acturs run fully concurrently
...
object BankAccount {
    ...
    case class Withdraw(amoutn: BigInt) {
        require(amount > 0) // always positive
    }
    case object Done
    case object Failed
...
class Manager extends Actor {
    override val supervisorStratefy = OneForOneStrategy() { // val not def (define it once)
