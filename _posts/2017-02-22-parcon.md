...
Again, the parallel reduction operator `g` and the neutral element `z` should form a [monoid](http://mathworld.wolfram.com/Monoid.html) (`z` should be the netral element of `g`). Note that this is *just* an "if", not "if and only if"; the only iff condition for `aggregate` to work is:
...
- Partitions never span multiple machines; tuples in the same partition are gauranteed to be on the same machine
- Each machine in the cluster contains one or more partitions
- THe numbe rof partitions to use is configurable. By default, it's the total number of cores on all executor nodes

There are two kinds of partitioning in Spark: 

- Hash partitioning: attempts to spread the data evenly across parrtitions *based on the keys* (`partition = k.hashCode() % numPartitions`)
...
Customizuing a partitioning is only possible on Pair RDDs. There are two ways to set partitioning for our data:
...
For partition resulting from transformations, the following operations on Pair RDDs hold to (and propagate) a partitioner:
...
- `partitionBy`
- `sort`
- It the parent has a partitioner: `mapValues`, `flapMapValues`, `filter`
...
Once we have a `DataFrame`, we can freely write SQL snytax. We just have to register it as a temporary SQL view first:
...
- Drop rows with unwanted values
    + `drop()` drops wors that contain `null` or `NaN` values in **any** column and returns a new DataFrame
