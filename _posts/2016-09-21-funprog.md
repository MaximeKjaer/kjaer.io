...
for (x <- e1 if f; s) yield e2 // where s is a (potentially empty) sequence of generators and filters
...
Interestingly, the translation of `for` is not limited to lists, sequences, or collections. Since it's based solely on the presence of the methods `map`, `flatMap` and `withFilter`, you can simply redefine these methods for your own types.

As long as these methods are defined, we can use the `for` syntax for querying the database.

### Functional Random Generators

#### Definition
We could also define these methods for a random value generator. Let's define it as such:
...
This means that ssing a lazy value for `tail`, `Stream.cons` can be implemented more efficiently:
...
We also don't need to worry too much aout infinite recursions with infinite streams since the tail isn't evaluated:
